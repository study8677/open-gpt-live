...
当前版本提供一个聚焦的 MVP：浏览器文本输入、按住说话、整段 STT 转写、内存会话历史，以及 LLM 流式回复。
...
- **模型可替换**：LLM 和 STT 都在 adapter 后面，先支持 OpenAI-compatible API。
- **会话感知**：每个 Gateway 连接维护自己的对话历史和活跃响应状态。
- **渐进式演进**：先跑通文本，再做按住说话，之后再进入 TTS 和更复杂的打断状态机。
...
| 中断 | LLM 流式阶段已完成 | 发送 `interrupt`，Gateway abort 当前 LLM 请求。 |
| TTS / 语音播放 | 未实现 | 下一阶段计划。 |
...
apps/web
  ...
  文本输入或按住说话 -> WebSocket -> 转写文本/流式回复展示

apps/gateway
  Node.js ws 服务
  音频聚合 -> STT -> 会话历史 -> OpenAI-compatible LLM -> 流式 delta
...
3. 输入文字并点击 `Send`。
4. 确认助手回复会逐字流式出现。
5. 再发送第二条文本，确认模型能引用上一轮上下文。
6. 按住 `Hold to Talk`，说一句话后松开。
7. 确认页面先显示转写文本，再流式显示助手回复。
8. 在浏览器中拒绝麦克风权限，确认页面有明确提示且文字输入仍可用。
9. 在回复流式输出时点击 `Stop`。
10. 确认流式输出立即停止。
...
预留事件包括 `transcript.partial` 和 `tts.chunk`。
...
当前里程碑刻意保持范围收敛：先把协议、整段转写、流式输出、上下文处理和中断语义打稳，再进入 TTS 和更实时的音频能力。
