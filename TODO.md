...
- [ ] 定义统一的语音请求时间点和指标结构。
- [ ] 浏览器使用单调时钟记录采集、事件到达和播放时间。
- [ ] Gateway 使用服务端单调时钟记录 STT、LLM 和 TTS 阶段时间。
- [ ] 不直接用客户端和服务端的绝对时间相减，避免时钟偏差。
- [ ] 为每条指标关联 `sessionId`、`requestId` 和 `turnMode`。
- [ ] 在 Gateway 结构化日志中输出阶段耗时，不记录原始音频和完整转写。
- [ ] 在页面增加轻量调试面板，显示当前请求的四项核心延迟。
- [ ] 中断或失败时保留已经产生的指标，并标记未完成阶段。
- [ ] 为指标计算、缺失事件和重复事件增加单元测试。
- [ ] 为文字、PTT、Live 和 Realtime fallback 增加集成测试。
- [ ] 更新配置文档和 Live Mode 验收清单。

### 验收标准

- [ ] 完成一次 Live 对话后，页面能显示四项核心延迟。
- [ ] 同一个请求在页面和日志中可以通过 `requestId` 对齐。
- [ ] 中断、断线和 Provider 失败不会产生负数或明显错误的耗时。
- [ ] Mock 测试可以使用可控时钟，结果稳定且不依赖真实 API。
...
- [ ] 在开启 Live Mode 后采集短时间环境噪声，计算初始噪声基线。
- [ ] 使用滑动窗口持续更新噪声基线，用户说话期间暂停或降低更新速度。
- [ ] 根据噪声基线计算动态 speech/silence threshold。
- [ ] 保留最小值、最大值和手动覆盖，防止阈值无限漂移。
- [ ] 保留开始/结束双阈值，避免状态在临界值附近反复切换。
- [ ] 在调试面板显示当前 RMS、噪声基线和动态阈值。

### 第二层：轮次确认

- [ ] 增加最短有效语音时长，过滤点击声、键盘声和瞬时噪音。
- [ ] 区分句中短暂停顿和真正结束，避免用户停顿一下就被抢话。
- [ ] 支持可配置的最短说话时长、停顿宽限和最大轮次时长。
- [ ] TTS 播放期间继续提高触发门槛，并保留播放结束抑制窗口。
- [ ] 为后续轻量语音分类模型预留可替换的 `SpeechDetector` 接口。
- [ ] 第一版不强制引入重量级 VAD 模型，先用采集数据验证是否需要第二模型。

### 测试 TODO

- [ ] 为动态阈值、噪声基线和状态迁移编写纯逻辑测试。
- [ ] 覆盖持续背景噪音、突然噪音、短暂停顿和长停顿。
- [ ] 覆盖播放中阈值提升、用户插话和播放结束抑制。
- [ ] 覆盖设备音量突然变化后的阈值恢复。
...
- [ ] 单次键盘敲击或短促噪音不会创建有效语音轮次。
- [ ] 正常句中停顿不会立即结束轮次。
- [ ] 用户真实插话仍能及时终止当前 TTS。
- [ ] 关闭动态模式后仍可回退到现有固定阈值行为。
